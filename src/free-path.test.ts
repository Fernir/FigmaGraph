...
import { buildFreePathPlan } from "./free-path.js";
...
describe("free path plan", () => {
  it("builds steps when node-id present", () => {
    ...
    assert.ok(plan.steps.some((s) => s.tool === "figmagraph_sync"));
    assert.ok(plan.minimumViable.includes("get_screenshot"));
  });
